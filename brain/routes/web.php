...
Route::get('/agent/lead/capture', function (\Illuminate\Http\Request $request) {
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO leads (external_lead_id, name, first_name, last_name, phone, email, address, city, state, zip_code, type, source, meta, tenant_id, created_at, updated_at) VALUES (:eid, :name, :first, :last, :phone, :email, :addr, :city, :state, :zip, 'auto', 'vicidial-iframe-capture', :meta, 5, NOW(), NOW())");
...
Route::get("/agent/lead-by-phone/{phone}", function($phone) {
    ...
    try {
        ...
        // No lead found in Brain - get Vici data and go to edit page
        $viciData = null;
        if ($viciLeadId) {
            try {
                // Use Vici proxy to get lead data
                $viciProxyUrl = "https://quotingfast-brain-ohio.onrender.com/vici-proxy/execute";
                $viciApiKey = "sk-KrtJqEUxCrUvYRQQQ8OKbMBmOa2OYnW5S5tPwPQJzIGBBgSZ";
